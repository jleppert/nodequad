...
/**
 * Events emitted during connection state changes. They can be monitored by attaching callbacks to the driver object: driver.on('state.disconnected', function() { .. });
	...
	if(uri) {
		...
		this.list(function checkURIByPortList(err, portList) {
			if(portList.filter(function(port) {
				port.name === uri.pathname ? true : false;
			}).length) {
				this.uri = this.portList.pop().uri;
			}
		});
	}
...
SerialDriver.prototype.connect = function connect(uri, cb) {
	var self = this;
	// use last probe if there is one
	var uri = uri ? uri : (this.foundPorts.length ? this.foundPorts[this.foundPorts.length-1] : undefined);
 ...
 * @param {Boolean} alsoConnect true to connect to first found vehicle
 */
SerialDriver.prototype.probe = function probe(alsoConnect) {
	var self = this;

	if(alsoConnect) {
		this.once('pong', function connectOnPong(port) {
			self.connect(port.uri, function autoConnectErrorCb(err) {
				alsoConnect.call(self, err, port);
			});
		});
	}
			...
			_connect(port.uri, function portProbeErrorCb(err, driver) {
				if(!err) {
					(function portConnectClosure(port) {
						...
						setTimeout(function portProbeTimeout() {
							driver.close();
